...
#define IMPORTMOD_VERSION		IMPORTMOD_VERSION_14
...
// pixel aspect ratio (new in 6.0)
	...
	csSDK_int32		colorProfileSupport;// Added in CS5.5. Currently only meaningful in After Effects. See the imColorProfileSupport enum
 	csSDK_int32		reserved[20];		// reserved; set to null
} imImageInfoRec;
...
typedef struct
{
	...
	char			alwaysUnquiet;		// If != 0, then Premiere will always unquiet this file immediately when the
										// application resumes.
	char			highMemUsage;		// If != 0, then MediaCore will limit the number of open file instances with this
										// flag set to true to 5. Only for use in extreme cases, like QT and Windows Media.
...
{	
	...
	csSDK_int64				outScale;				// numerator of the time scale(e.g. 30000)
	csSDK_int64				outSampleSize;			// denominator of the time scale (e.g. 1001)
