...
segmentInfo::segmentInfo()
{
	...
	playcode = PRT_PLAYCODE_REALTIME;
	clipID = -1;
}
...
prMALError VideoSequenceParser::ParseSequence (PrTimelineID mSequenceID, csSDK_int32 indentation)
{
	...
	for (int i = 0; i < numSegments; i++, segmentIt++)
	{
		...
		if (strcmp (segmentIt->segmentHash.mGUID, segmentHash.mGUID))
		{
			...
			#endif
			segmentIt->playcode = PRT_PLAYCODE_REALTIME;  // RT until proven otherwise
			...
			else if (strcmp(nodeType, kVideoSegment_NodeType_Preview) == 0)
			{
				...
				// Preview files should be marked as non-realtime (even though they should play realtime)
				segmentIt->playcode = PRT_PLAYCODE_NON_REALTIME_UNSPECIFIED;
	...
	else
	{
		...
		segmentEnd->playcode = PRT_PLAYCODE_REALTIME;
	...
	csSDK_int32							indentation)
{
	...
	// This player chooses to mark all transition segments as non-RT
	segmentIt->playcode = PRT_PLAYCODE_NON_REALTIME_UNSPECIFIED;
	...
	csSDK_int32							indentation)
{
	...
	else
	{
		...
		else
		{
			...
			// If media doesn't match sequence size, or speed is not 100%
			if (mediaWidth != mWidth || 
				mediaHeight != mHeight ||
				mediaPARNum != mPARNum ||
				mediaPARDen != mPARDen)
			{
	...
	csSDK_int32							indentation)
{
	...
	for (int operatorNodeIndex = 0; operatorNodeIndex < clipNodeOperators; operatorNodeIndex++)
	{
		...
		// This player chooses to mark all segments with clip operators as non-RT
		segmentIt->playcode = PRT_PLAYCODE_NON_REALTIME_UNSPECIFIED;
...
void VideoSequenceParser::GetRTStatus (
	PrTime		startTime,
	PrTime		&endTime,
	prtPlaycode	&playcode)
{
	...
	if (startTime == -kPrSDKEndOfTimeline &&
		mCutlist->begin()->startTime != -kPrSDKEndOfTimeline)
	{
		endTime = mCutlist->begin()->startTime;
		playcode = PRT_PLAYCODE_REALTIME;
	}
	else
	{
		findSegmentResult = CutlistFindSegmentAtTime(startTime, &segmentIt);
		if (findSegmentResult == kFindSegmentSuccess)
		{
			endTime = segmentIt->endTime;
			playcode = segmentIt->playcode;
		}
		else if (findSegmentResult == kFindSegmentMissing)
		{
			endTime = segmentIt->startTime;
			playcode = PRT_PLAYCODE_REALTIME;
		}
		else if (findSegmentResult == kFindSegmentEnd)
		{
			endTime = kPrSDKEndOfTimeline;
			playcode = PRT_PLAYCODE_REALTIME;
		}
		...
		{
			endTime = kPrSDKEndOfTimeline;
			playcode = PRT_PLAYCODE_NON_REALTIME_UNSPECIFIED;
		}
	}
}
